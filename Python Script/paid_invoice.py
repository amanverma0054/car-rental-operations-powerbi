...
ONEDRIVE_PATH = r"C:\Users\lenovo\OneDrive\API Call\paid_invoice.xlsx"
...
if __name__ == "__main__":
    ...
    for chunk_start, chunk_end in daterange_chunks(start_date, end_date, 7):
        ...
        body = {
            "invoicePaid": "paid",     # 👈 only PAID invoices
            "dateFilter": "date",
            "dateRange": {
                "start": start_str,
                "end": end_str
        ...
        if data:
            for row in data:
                row["invoicePaid"] = "paid"  # add status column
    ...
    if all_results:
        ...
        print(f"\n✅ Saved {len(all_results)} PAID invoices to OneDrive: {ONEDRIVE_PATH}")
    else:
        print("\n❌ No PAID invoices fetched from API.")
